...
import { ResultCollectionJsonOutput } from "../types/Interfaces/Json/ResultCollectionJsonOutput";
...
class RollResults implements ResultCollection {
   ...
   */
  constructor(rolls: (SingleResultJsonOutput|SingleResult|number)[] = []) {
    this.rolls = rolls;
  }
   ...
   * @throws {TypeError} Rolls must be an array
   */
  set rolls(rolls: (SingleResultJsonOutput|SingleResult|number)[]) {
   ...
   */
  addRoll(value: SingleResultJsonOutput|SingleResult|number): void {
    const result = (value instanceof RollResult)
      ? value :
      new RollResult(value);
  ...
  toJSON(): ResultCollectionJsonOutput {
    ...
    return {
      rolls: rolls.map<SingleResultJsonOutput>((roll: SingleResult) => roll.toJSON()),
