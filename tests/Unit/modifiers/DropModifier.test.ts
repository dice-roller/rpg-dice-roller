...
describe('DropModifier', () => {
  describe('Initialisation', () => {
    test('model structure', () => {
      const mod = new DropModifier(RangeEnd.Low);

      expect(mod).toBeInstanceOf(DropModifier);
      expect(mod).toBeInstanceOf(Modifier);

      expect(mod).toHaveProperty('end', RangeEnd.Low);
      expect(mod).toHaveProperty('name', 'drop-l');
      expect(mod).toHaveProperty('notation', 'dl1');
      expect(mod).toHaveProperty('order', 7);
  ...
  describe('End', () => {
    test('setting in constructor calls setter', () => {
      ...
      // create the modifier
      new DropModifier(RangeEnd.Low);

      expect(spy).toHaveBeenCalledTimes(1);
      expect(spy).toHaveBeenCalledWith(RangeEnd.Low);

      // create the modifier
      new DropModifier(RangeEnd.High);
    ...
    test('can be changed', () => {
      const mod = new DropModifier(RangeEnd.High);
  ...
  describe('Quantity', () => {
    test('qty must be numeric', () => {
      const mod = new DropModifier(RangeEnd.Low, 8);
    ...
    test('qty must be positive non-zero', () => {
      let mod = new DropModifier(RangeEnd.Low, 1);
      expect(mod.qty).toBe(1);
      expect(mod.notation).toEqual('dl1');

      mod = new DropModifier(RangeEnd.Low, 324);
    ...
    test('float gets floored to integer', () => {
      let mod = new DropModifier(RangeEnd.High, 5.145);
      expect(mod.qty).toBeCloseTo(5);
      expect(mod.notation).toEqual('dh5');

      mod = new DropModifier(RangeEnd.High, 12.7);
      expect(mod.qty).toBeCloseTo(12);
      expect(mod.notation).toEqual('dh12');

      mod = new DropModifier(RangeEnd.High, 50.5);
    ...
    test('must be finite', () => {
      expect(() => {
        new DropModifier(RangeEnd.High, Infinity);
      }).toThrow(RangeError);
    });

    test('can be very large number', () => {
      const mod = new DropModifier(RangeEnd.High, 99 ** 99);
  ...
  describe('Notation', () => {
    test('simple notation', () => {
      let mod = new DropModifier(RangeEnd.Low, 35);
      expect(mod.notation).toEqual('dl35');

      mod = new DropModifier(RangeEnd.High, 90876684);
      expect(mod.notation).toEqual('dh90876684');

      mod = new DropModifier(RangeEnd.High, 7986);
      expect(mod.notation).toEqual('dh7986');

      mod = new DropModifier(RangeEnd.Low, 2);
  ...
  describe('Output', () => {
    test('JSON output is correct', () => {
      const mod = new DropModifier(RangeEnd.High, 4);
    ...
    test('toString output is correct', () => {
      const mod = new DropModifier(RangeEnd.Low, 4);
  ...
  describe('Run', () => {
    let mod: DropModifier;
    let results: ResultGroup|RollResults;

    beforeEach(() => {
      mod = new DropModifier(RangeEnd.Low);
    });
