...
describe('ResultGroup', () => {
  ...
  let results: (RollResults|string|number)[];
  let subRollResults: ResultGroup[];
  ...
  describe('Use In Total', () => {
    ...
    test('cast to boolean', () => {
      ...
      // `false` because the setter takes the value directly
      // @ts-expect-error testing with falsey value
  ...
  describe('Output', () => {
    test('JSON output is correct', () => {
      ...
      expect(json.results).toEqual([
        ...group
          .results
          .map((result) => (typeof result === 'object' ? result.toJSON() : result))
      ]);
      expect(json.type).toEqual(ModelType.ResultGroup);
    ...
    test('toString output modifiers with multiple sub-rolls', () => {
      group.isRollGroup = true;
      group.results = subRollResults;
      (group.results[0] as ResultGroup).modifiers = ['drop'];
