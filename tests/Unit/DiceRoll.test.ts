...
describe('DiceRoll', () => {
  ...
  describe('Notation', () => {
    test('cannot be changed', () => {
      ...
      expect(() => {
        // @ts-expect-error test cannot set readonly
  ...
  describe('Output', () => {
    ...
    describe('toString', () => {
      test('returns notation and rolls as a string', () => {
        ...
        expect(diceRoll.toString()).toEqual(`${notation}: [6, 2, 5, 8] = 21`);
      ...
      test('calls RollResults toString', () => {
        ...
        expect(diceRoll.toString()).toEqual(`${notation}: [6, 2d, 5d, 8]*(5+[3d, 9]) = 196`);
        expect(spy).toHaveBeenCalledTimes(2);
      ...
      test('returns success / failure count as total', () => {
        ...
        expect(diceRoller.toString()).toEqual(`${notation}: [6*, 2_, 5, 8*] = 1`);
      ...
      test('can handle success / failure combined with normal rolls', () => {
        ...
        expect(diceRoller.toString()).toEqual(`${notation}: [6*, 2_, 5, 8*]+[3, 5] = 9`);
      ...
      test('can handle math only roll', () => {
        ...
        expect(diceRoll.toString()).toBe('4+8: 4+8 = 12');
      });
      ...
      test('returns "No dice rolled" if no rolls', () => {
        ...
        expect(diceRoller.toString()).toEqual(`${notation}: No dice rolled`);
  ...
  describe('Export', () => {
    ...
    test('can export to valid JSON', () => {
      const exported = diceRoll.export(ExportFormat.Json);

      expect(exported).toEqual(JSON.stringify(diceRoll));
    });

    test('can export to valid base64', () => {
      const exported = diceRoll.export(ExportFormat.Base64) as string;
    ...
    test('can export to plain object', () => {
      const exported = diceRoll.export(ExportFormat.Object);
  ...
  describe('Import', () => {
    ...
    test('can import JSON encoded string', () => {
      const exported = diceRoll.export(ExportFormat.Json) as string;
      const importedRoll = DiceRoll.import(exported);

      expect(importedRoll).toBeInstanceOf(DiceRoll);
      expect(importedRoll.export(ExportFormat.Json)).toEqual(exported);
    });

    test('can import base64 encoded object', () => {
      const exported = diceRoll.export(ExportFormat.Base64);
      const importedRoll = DiceRoll.import(exported);

      expect(importedRoll).toBeInstanceOf(DiceRoll);
      expect(importedRoll.export(ExportFormat.Base64)).toEqual(exported);
    });
    ...
    test('can import plain object', () => {
      ...
      expect(importedRoll).toBeInstanceOf(DiceRoll);
      expect(importedRoll.export(ExportFormat.Object)).toEqual(exported);
    });
